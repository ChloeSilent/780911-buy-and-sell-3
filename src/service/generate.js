...
const createArray = (arr, arrLength) => {
  return arr
    .slice()
    .sort(function () {
...
const createMockArray = (numb) => {
  ...
  for (let i = 0; i < numb; i++) {
    mockedArr.push({
      ...
      description: createArray(mocksDescription, 4).join(),
      sum: randomIntFromInterval(1000, 100000),
      picture: `item${zeroFill(
        randomIntFromInterval(1, MAX_PHOTO_IMAGE_NUMBER),
        2
      )}.jpg`,
      category: createArray(mockCategory, mockCategory.length - 1),
    });
  }
...
const generate = (input) => {
  ...
  if (Number(input) && input.trim() !== "") {
    ...
    mockArr = createMockArray(Number.parseInt(input, 10));
  } else {
